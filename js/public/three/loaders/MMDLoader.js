...
THREE.MMDAudioManager = function ( audio, listener, p ) {
	...
	this.durationBlack = this.duration;
...
THREE.MMDAudioManager.prototype = {
	...
	control: function ( delta ) {
		...
		// this.currentTime 为 ready = ture 时开始计算 不是歌曲播放的时间
		this.elapsed += delta;
		this.currentTime += delta;

		//console.log(this.currentTime)
		if ( this.checkIfPauseAudio() ) {
		...
		if ( this.checkIfStartAudio() ) {
			...
			//console.log("durationBlack"+ this.durationBlack + "duration"+ this.duration );	
			//音频准备播放 
			//当结束时，如果动画模型没有结束，则一直执行该函数，因为isPlaying为fasle 但是不会播放歌曲 因为 startiome > audio.duration 从超过音频本身的点开始播放就没声音 而且停止函数this.audio.stop();需要source.start()执行
	...
	checkIfStartAudio: function () {
		...
		if ( this.currentTime < this.delayTime ) {
			...
			return false;
			console.log("currentTime < delayTime")
...
THREE.MMDHelper.prototype = {
	...
	unifyAnimationDuration: function ( params ) {
		...
		//**addif()
		try{
			var tempstr = "全模块最大时间:"+Math.round(max)+"s";
